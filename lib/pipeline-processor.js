...
class PipelineProcessor {
     ...
     * @param {Array} param1.searchPaths Additional search paths to prepend to module paths. (HINT: include the application root and use ./ to start the path)
     */
    constructor(logger, { source = false, transformers = [], loader = false, searchPaths = [] } = {}) {
        
        if (!Array.isArray(searchPaths)) {
            throw new Error("searchPaths must be an array");
        }
    ...
    async loadPipelineStep(expectedClass, stepmodule, stepConfig) {
        ...
        if (typeof stepmodule === 'string') {
            ...
            //We want to allow use of relative paths to modules of a program
            //making use of our pipeline. However, not search paths require 
            //either absolute paths or the required items to be in a node_modules
            //folder. (Or relative to this module in a node_modules folder...)
            if (stepmodule.startsWith(".") && this.searchPaths.length) {
                
                for (let spath of this.searchPaths) {                    
                    const modulePath = path.join(spath, stepmodule);

                    this.logger.debug(`Searching ${modulePath}.`);
            ...
            // Load the module using default search paths
            if (!loadedModule) {
                try {
                    loadedModule = require(stepmodule);
                } catch(err) {
                    this.logger.debug(`Could not find step, ${moduleName}, in default search path.`);
                }
            }

            if (!loadedModule) {
                this.logger.error(`Could not load step, ${moduleName}.`);
                throw new Error(`Could not load step, ${moduleName}.`);
            }
